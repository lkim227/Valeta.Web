module AppCommon {
    export class BillingDocumentController extends AppCommon.ControllerBase {
        ...
        constructor(
            ...
            if (!!this.ticketIdentifier && this.ticketIdentifier.length > 0) {
                const ordersPromise = this.billingDocumentForTicketQuery.getBillingDocument(this.ticketIdentifier);
                ordersPromise.then((result) => {
                    if (result) {
                        const urlPromise = this.billingDocumentForTicketQuery.getBillingDocumentUrl(this.ticketIdentifier);
                        urlPromise.then((resultUrl) => {
                            if (!!resultUrl && resultUrl.length > 0) {
                                ...
                                $("#billingDocumentReport").load(`${this.billingDocumentUrl} div`);  // only load div elements so head and body elements are not included
        ...
        printReport = (): void => {
                ...
                frameEstimate.name = "BillingDocument";
                ...
                setTimeout(function () {
                    window.frames["BillingDocument"].focus();
                    window.frames["BillingDocument"].print();
    ...
    angular.module("app.common")
        .controller("BillingDocumentController",
        [
            "$stateParams", "AuthService", "BillingDocumentForTicketQuery",
            (sparams, auth, est) => new BillingDocumentController(sparams, auth, est)
        ]);
}