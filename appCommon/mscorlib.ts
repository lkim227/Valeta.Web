...
class Type extends NObject {
    ...
    Equals(obj: any): boolean {
        return (obj instanceof Type) && ((obj as Type).Name === this.Name);
    }
}
...
class CultureInfo extends NObject implements IFormatProvider {
    static InvariantCulture = new CultureInfo("Invariant");
    static CurrentCulture = CultureInfo.InvariantCulture;

    Name = "Invariant";

    private nfi = new NumberFormatInfo();
...
class List<T> extends NObject implements IList<T>, IEnumerable<T> {
    array = new Array<T>(); // Public to help the enumerator
...
class Dictionary<K, V> extends NObject implements IDictionary<K, V>, IEnumerable<KeyValuePair<K, V>> {
    ...
    get_Item(key: K): V {
        return this.values[this.GetKeyString(key)] as V;
    }
